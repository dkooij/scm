...
def compute_page_changes_per_day():
    changes_per_day, start_date = [], None

    with open(INPUT_FILE_PATH) as input_file:
        for line in input_file:
            [_, date_str, change_str] = line.strip().split(",")
            date_obj = datetime.strptime(date_str, "%Y%m%d")
            ...
            while day_number >= len(changes_per_day):
                changes_per_day.append(0)
            changes_per_day[day_number] += int(change_str)
...
cpd = [37795, 38817, 43266, 43021, 42565, 42389, 41933, 37722, 39007, 43525, 42689, 42605, 43306, 42355, 37751, 38817, 42932, 43545, 43871, 44311, 42110, 37646, 38696, 42503, 43061, 42316, 42669, 41716, 37792, 37953, 42346, 42760, 42262, 42160, 40854, 36649, 37690, 42155, 41886, 41201, 41449, 40275, 36561, 37632, 41305, 41307, 40246, 40788, 40238, 37620, 38222, 41042, 40914, 40538, 40717, 40168, 36978, 37781, 52759, 50954, 39710, 40090, 39407, 9251, 9322, 41881, 16445, 16617, 41499, 16254, 14891, 38200, 41604, 41589, 41565, 41779, 41233, 37845, 39221, 19291, 19835, 42345, 42721, 41390, 37379, 38891, 43367, 42586, 41791]

draw_alluvial_plot(cc)
draw_page_changes_per_day(cpd)
draw_different_change_behaviour_fractions(cc)
draw_same_change_behaviour_fractions(cc)
draw_fraction_not_changed_weekly(cm)
draw_fraction_changed_every_day_weekly(cm)
